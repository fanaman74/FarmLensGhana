...
export default function CropFinder() {
  ...
  return <div className="crop-finder">
    <section className="panel panel-pad finder-form">
      ...
      <button className="btn btn-primary" disabled={!place || !crop} onClick={() => { if (place && crop) { setPolygon([]); setEarthLayer(null); setSearch({ place, crop }); } }}>Explore crop area</button>
    </section>
    ...
    {search && <div className="soil-grid">
      <section className="panel finder-map"><FarmMap key={`${search.place.id}-${search.crop.slug}`} latitude={search.place.latitude} longitude={search.place.longitude} zoom={12} drawing cropland rasterLayer={earthLayer} onPolygon={setPolygon} /></section>
