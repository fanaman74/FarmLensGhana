...
export const GET: APIRoute = async ({ url }) => {
  ...
  return Response.json(result, { status: result.ok ? 200 : result.error.code === 'timeout' ? 504 : 502, headers: { 'Cache-Control': result.ok ? 'public, max-age=300, stale-while-revalidate=300' : 'no-store' } });
};
